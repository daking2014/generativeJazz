...
class DCGAN(object):
  ...
  def train(self, config):
    ...
    elif config.dataset == 'jazzyjazz':
      data = np.load(os.path.join('./data', config.dataset, 'chordNoteAttackSmall.npz'))['data']
      data = data.reshape((300, 96, 50, 1)).tolist()
    else:
    ...
    if config.dataset == 'mnist':
      ...
      data = np.array(data)
      # print(data.shape)
      # data = np.array(data).reshape((300, 192, 15, 1))
      ...
      for idx in xrange(0, batch_idxs):
        ...
        if config.dataset == 'mnist':
          ...
          errG = self.g_loss.eval({
              ...
              self.y: batch_labels
                    ...
                    # Update D network
          for i in range(int(np.ceil(errD_fake+errD_real))):
            _, summary_str = self.sess.run([d_optim, self.d_sum],
              feed_dict={
                self.inputs: batch_images,
                self.z: batch_z,
          ...
          # Run g_optim twice to make sure that d_loss does not go to zero (different from paper)
          for i in range(int(np.ceil(errG))*3):
            _, summary_str = self.sess.run([g_optim, self.g_sum],
        ...
        if idx == batch_idxs-1:
          if config.dataset == 'mnist':
            ...
            for i in range(2):
              np.save('./{}/second_{:02d}_{:04d}_{:02d}.npy'.format(config.sample_dir, epoch, idx, i), samples[i, :, :, :])
            print("[Sample] d_loss: %.8f, g_loss: %.8f" % (d_loss, g_loss))
            # except:
            #   print("one pic error!...")
