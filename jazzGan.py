...
D_STEPS = 1  # how many times to train the discriminator per generator step
LEARNING_RATE = 0.01 * SEQ_LENGTH
SEED = 88

DATAFILE = 'justNotesAdjusted.npz'
...
def verify_sequence(seq):
    """ Do we really need a function that verifies the sequence in some way? """
    return True


def get_random_sequence():
    """Gets a valid sequence of notes from training set"""
    loadedFile = np.load(DATAFILE)
    trainingSeq = loadedFile['data'].tolist()
    randSeq = random.choice(trainingSeq)
